...
Wormandtomato <- read_excel("Wormandtomato.xlsx", 
...
tomato <- Wormandtomato%>% 
  ...
  filter(!is.na(`!Notes:InChIKey_neutral`),
         !is.na(`!Notes:ChEBI_neutral`), !is.na('!Notes:Formula_Neutral'))

# database request for InChIkeys to SMILES -from anusha
gsmiles=c()

for (i in 1:length(tomato$'!Notes:InChIKey_neutral')){
  tes<-tryCatch({webchem::get_cid(stringr::str_trim(as.character(tomato$`!Notes:InChIKey_neutral`[i])), from = "inchikey")},error=function(cond){message("webchem not able to get cid from Inchikey")})
  ...
  tes2IN<-tryCatch({tes2$InChI},error=function(cond){message("Inchi to Inchikey failed because of CID not converting")})
...
tomato_adducts_long <- pivot_longer(tomato_adducts, cols = -colnames(tomato)) %>% 
   rename(adduct = name,
          mz = value) %>% 
...
homol <- homol.search(tomato_Dino,
                      isotopes,	elements = c("C", "H", "O"), use_C = T, # 
                      minmz=10, 	maxmz=35,
                      minrt=-.5,  maxrt=2, #0
                      ppm=T,
                      mztol=5,  rttol=0.5, # 0.5
                      minlength=3, #5
                      mzfilter=F,
                      spar=.45, 	R2=.98,
                      plotit=F) # do the thing...
...
homol_filter <- homo_tibble %>%
 ...
 #write.csv(homol_filter, file = "simulate_lcms_tomato_homolseries.txt",sep = "\t", row.names = T, col.names = T, dec = ",")
...
tomato_reunited <- merge(homol_filter, tomato_adducts_long, by.x = c('mz'), by.y= "mz")
